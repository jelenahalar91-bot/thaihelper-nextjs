...
function buildPages() {
  ...
  for (const city of CITIES_DATA) {
    pages.push({
      ...
      description: `Find verified household helpers in ${city.name}, Thailand. Browse nannies, housekeepers, private chefs, drivers, and more on ThaiHelper — free, no agency fees.`,
  ...
  for (const city of CITIES_DATA) {
    for (const cat of CATEGORIES_DATA) {
      ...
      pages.push({
        ...
        description: `Find verified ${cat.name.toLowerCase()} profiles in ${city.name}, Thailand. Salary range: ${adjustedMin.toLocaleString()}–${adjustedMax.toLocaleString()} THB/month. Direct hire, no agency fees.`,
