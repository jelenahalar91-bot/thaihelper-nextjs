...
// Reduce either a slug ("chiang_mai"), a display name ("Chiang Mai") or a
// sloppy legacy value ("Bangkok ") to the canonical slug. This is the only
// safe way to compare a helper's city with an employer's: helper_profiles
// store slugs (the register form submits CITY_OPTIONS[].slug) while
// employer_accounts store display names (that form submits CITIES, the name
// list). A raw `!==` between the two sides never matches.
export function toCitySlug(value) {
  if (!value) return '';
...
// Every spelling of a city that may be sitting in a `city` column, for use
// in a Supabase `.in('city', …)` filter. Postgres can't call toCitySlug(),
// so we widen the query and narrow again in JS with toCitySlug().
export function cityQueryVariants(value) {
  const slug = toCitySlug(value);
  if (!slug) return [];
  const known =
    CITY_OPTIONS.find(c => c.slug === slug) ||
    THAI_PROVINCES.find(p => p.slug === slug);
  const out = new Set([slug, String(value).trim()]);
  if (known) {
    out.add(known.name);
    out.add(known.name.toLowerCase());
  }
