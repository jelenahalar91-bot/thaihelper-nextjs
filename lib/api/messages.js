...
export async function fetchMessages(conversationId, page = 1, role) {
  const params = new URLSearchParams({ conversation_id: conversationId, page: String(page) });
...
export async function sendMessage(conversationId, content, role) {
  const url = role ? `/api/messages?role=${encodeURIComponent(role)}` : '/api/messages';
  const res = await fetch(url, {
    method: 'POST',
 ...
 * On failure throws an Error whose `.code` is the server error string
 * (e.g. 'email_not_verified') so callers can show the right prompt.
 */
export async function startConversation(helperRef) {
  const res = await fetch('/api/conversations?role=employer', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    const err = new Error(body.error || 'Failed to start conversation');
    err.code = body.error;
    throw err;
  }
...
export async function startConversationAsHelper(employerRef) {
  const res = await fetch('/api/conversations?role=helper', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
 ...
 */
export async function deleteConversation(conversationId, role) {
