...
import { isRefSuspended } from './suspension';
...
function parseCookie(req, name) {
  ...
  // Anchor the name to a cookie boundary. An unanchored `name=` matches in
  // the MIDDLE of a longer cookie name too, so any future cookie whose name
  // ends in one of ours (e.g. `legacy_th_session`) would silently win the
  // match and hand back the wrong token. None of the three current names
  // collide, but the failure mode is a session mix-up, so pin it now.
...
async function sessionFromCandidates(tokens, role) {
  for (const token of tokens) {
    ...
    if ((payload.role || 'helper') === role) {
      // A suspension has to reach sessions that already exist, not just the
      // next login — with SESSION_DAYS = 365 the "next login" may be a year
      // away. A suspended account's token is treated as no token at all, so
      // every caller's existing not-logged-in path handles it; the login page
      // it lands on then refuses with account_suspended and says why.
      //
      // Only the ref is checked, never the role: refs are prefixed (TH- /
      // EMP-) and unique across both tables, and a role mix-up here would
      // fail open. See lib/suspension.js for the 60s cache and its limits.
      if (await isRefSuspended(payload.ref)) return null;
