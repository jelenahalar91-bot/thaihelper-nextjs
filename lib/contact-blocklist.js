...
// "14000-15000", "20000 - 25000", "15 000 - 18 000" — a salary range, which is
// the single most common thing anyone negotiates in these chats. Concatenated
// it reads as a plausible 10-digit number, so without this it normalizes to a
// phone handle: 2 of the 60 phone matches in the September 2026 message corpus
// were ranges, not numbers.
//
// Two groups of 4-6 digits either side of a dash is the range shape; a real
// number split that evenly is not a spelling anyone uses. A leading + says
// "phone" outright and always wins.
function looksLikeNumberRange(raw) {
...
function normalizePhone(raw) {
  if (looksLikeNumberRange(raw)) return null;
...
/**
 * Only the handles that arrived as a TAP-TO-OPEN LINK, not as typed digits.
 *
 * The distinction carries the whole signal. "0812345678" in the middle of a
 * negotiation is how hiring ends on a direct-connection platform. A line.me
 * URL is a one-tap exit from the platform, which is what a handoff scam needs
 * and what an ordinary family has no reason to send — across 30 days of real
 * traffic (measured 2026-09-15) every single link an EMPLOYER sent came from
 * EMP-B4MUCP or EMP-3THHAA, both confirmed scams, and none from anyone else.
 *
 * Deliberately built on the same three regexes extractHandles uses, so the
 * alert and the blocklist can never drift apart on what a link looks like.
 *
 * @returns {string[]} canonical handles, deduplicated
 */
export function linkHandles(text) {
  if (typeof text !== 'string' || !text) return [];
  const found = new Set();
  for (const [, id] of text.matchAll(LINE_RE)) {
    const v = cleanId(id);
    if (v) found.add(`line:${v}`);
  }
  for (const [, id] of text.matchAll(TELEGRAM_RE)) {
    const v = cleanId(id);
    if (v) found.add(`telegram:${v}`);
  }
  for (const [, num] of text.matchAll(WHATSAPP_RE)) {
    const v = normalizePhone(num);
    if (v) found.add(`phone:${v}`);
  }
  return [...found];
}
