...
// Google's Translate v2 quota ("queries per 100 seconds per user") returns a
// 403 userRateLimitExceeded under normal registration bursts — a handful of
// signups within a few seconds is enough to trip it. That's transient, so we
// retry with backoff instead of permanently storing untranslated text (which
// is what happened silently for ~40% of Thai-named helpers before this fix).
...
function isRateLimitStatus(status, bodyText) {
  ...
  if (status === 403) return /rateLimitExceeded/i.test(bodyText || '');
  return false;
}
  ...
  for (let attempt = 0; attempt <= RATE_LIMIT_RETRIES; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        if (isRateLimitStatus(response.status, bodyText) && attempt < RATE_LIMIT_RETRIES) {
          console.warn(`Translation rate limited, retrying in ${RATE_LIMIT_BACKOFF_MS[attempt]}ms`);
          await sleep(RATE_LIMIT_BACKOFF_MS[attempt]);
          continue;
        }
        console.error('Translation API error:', response.status, bodyText);
        return null;
      }
