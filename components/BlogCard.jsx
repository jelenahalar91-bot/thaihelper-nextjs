...
const CATEGORY_LABELS = {
  families: { en: 'FAMILIES', th: 'ครอบครัว' },
  helpers: { en: 'HELPERS', th: 'ผู้ช่วย' },
};

export default function BlogCard({ post }) {
  const { lang } = useLang();
  const cat = CATEGORY_LABELS[post.category] || CATEGORY_LABELS.families;
  const title = (lang === 'th' && post.title_th) || post.title;
  const description = (lang === 'th' && post.description_th) || post.description;
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="flex items-center gap-2 text-xs text-gray-400">
              <time dateTime={post.date}>
                {new Date(post.date).toLocaleDateString(lang === 'th' ? 'th-TH' : 'en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
              </time>
